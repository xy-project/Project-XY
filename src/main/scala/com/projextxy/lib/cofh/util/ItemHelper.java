...
import com.projextxy.lib.cofh.api.item.IEmpowerableItem;
import com.projextxy.lib.cofh.api.item.IInventoryContainerItem;
import com.projextxy.lib.cofh.util.oredict.OreDictionaryProxy;
...
public final class ItemHelper {
    ...
    public static void addInventoryInformation(ItemStack stack, List<String> list, int minSlot, int maxSlot) {
        ...
        if (!stack.stackTagCompound.hasKey("Inventory") || stack.stackTagCompound.getTagList("Inventory", stack.stackTagCompound.getId()).tagCount() <= 0) {
            list.add(StringHelper.localize("info.lib.cofh.empty"));
            return;
        }
        ...
        if (containedItems.size() > 0) {
            list.add(StringHelper.localize("info.lib.cofh.contents") + ":");
        }
    ...
    public static void addAccessibleInventoryInformation(ItemStack stack, List<String> list, int minSlot, int maxSlot) {
        ...
        if (containedItems.size() > 0) {
            list.add(StringHelper.localize("info.com.cofh.contents") + ":");
        } else {
            list.add(StringHelper.localize("info.com.cofh.lib.cofh.empty"));
        }
        for (ItemStack item : containedItems) {
            int maxStackSize = item.getMaxStackSize();
